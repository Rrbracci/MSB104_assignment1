...
# Fourthly, we exclude the geom_smooth and instead use geom_line since this gives a accurate description of our data from Finland
#| cap-location: bottom
#| fig-cap: "GDP per year."
GDP_FI_nuts2 |>
  ggplot(mapping = aes(
...
# Fourthly, we exclude the geom_smooth and instead use geom_line since this gives a accurate description of our data from Romania
#| cap-location: bottom
#| fig-cap: "GDP per year."
GDP_RO_nuts2 |>
  ggplot(mapping = aes(
...
Gini_stat_DE_nuts2 <- merged_temp %>%
  ...
  group_by(Year) |> 
  summarise(
    min_Gini_each_year = min(gini_n2),
    max_Gini_each_year = max(gini_n2),
    median_Gini_each_year = median(gini_n2),
    mean_Gini_each_year = mean(gini_n2),
    sd_Gini_each_year = sd(gini_n2),
    var_Gini_each_year = sd_Gini_each_year^2
  )
```
...
Gini_stat_DE_nuts2 |> 
  ungroup() |> 
  select(Year, min_Gini_each_year, max_Gini_each_year, median_Gini_each_year, mean_Gini_each_year, sd_Gini_each_year, var_Gini_each_year) |> 
  st(group = "Year", group.long = TRUE, 
     summ = c('notNA(x)','mean(x)', 'median(x)', 
              'sd(x)','min(x)','max(x)'),
     )
